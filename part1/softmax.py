...
def compute_probabilities(X, theta, temp_parameter):
    ...
    # For numerical stability, subtract the max logit value from each column
...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    # Create a one-hot encoded matrix for the labels Y
    k = theta.shape[0]
...
def run_gradient_descent_iteration(X, Y, theta, alpha, lambda_factor, temp_parameter):
    ...
    # Create a sparse matrix for the one-hot encoded labels
    one_hot_Y = sparse.coo_matrix((np.ones(n), (Y, np.arange(n))), shape=(k, n)).toarray()

    # Compute the gradient of the loss function
    grad = (-1 / (temp_parameter * n)) * ((one_hot_Y - probabilities) @ X) + lambda_factor * theta
...
def compute_test_error_mod3(X, Y, theta, temp_parameter):
    ...
    """
    #YOUR CODE HERE
    Y_predict=get_classification(X, theta, temp_parameter)
    Y_predict_mod3 = np.mod(Y_predict,3)
    return 1-np.mean(Y_predict_mod3==Y)
